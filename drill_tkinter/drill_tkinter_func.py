...
def findDir1(self):
    ...
    self.txtFile1.insert(0,filePath)

def findDir2(self):
    filePath = tk.filedialog.askdirectory()
    self.txtFile2.insert(0,filePath)
    

# Select item in ListBox
def onSelect(self, event):
    # calling the event is the self.
...
def addToList(self):
    ...
    var_lname = var_lname.strip() # This will remove any blank spaces before and after the user's entry
    var_lname = var_lname.title() # This will ensure that the first character in each word is capitalized
    var_fullname = ("{} {}".format(var_fname, var_lname)) # combine our normailized names into a fullname
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            cursor = conn.cursor()
            # Check the database for existence for the fullname, if so we will alert user and disregard request
...
def onUpdate(self):
    ...
    var_email = self.txt_email.get().strip() # normalize the data to maintain database integrity
